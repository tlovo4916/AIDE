...
class ContextBuilder:
    ...
    async def build(
        ...
        budget.allocate(
            "Cross-Agent Context", cross_ctx, max_ratio=settings.cross_ratio,
        )

        lit_ctx = await self._build_literature_context(task, knowledge_base)
        budget.allocate(
            "Literature Context", lit_ctx, max_ratio=settings.literature_ratio,
        )

        history_ctx = await self._build_history_context(agent_role, board)
        budget.allocate(
            "History Context", history_ctx, max_ratio=settings.history_ratio,
        )
    ...
    async def _build_core(self, board: Blackboard) -> str:
        ...
        project_l0 = (
            ...
            f"Phase: {meta.get('phase', 'explore')}"
        )
        decisions = await board.get_decisions()
        if decisions:
            recent = decisions[-5:]
            digest = "\n".join(
                f"- {d.chosen}: {d.rationale[:100]}" for d in recent
            )
        else:
    ...
    async def _build_task_context(
        self, agent_role: AgentRole, board: Blackboard,
    ) -> str:
        parts: list[str] = []
        ...
        for at in own_types:
            ...
            for m in metas:
                ...
                content = await self._load_and_track(
                    board, at, m.artifact_id, ver, ContextLevel.L2,
                )
                if content:
                    parts.append(
                        f"[{at.value}/{m.artifact_id} v{ver}]\n{content}"
                    )
        ...
        for at in dep_types:
            ...
            for m in metas:
                ...
                if ver == 0:
                    continue
                content = await self._load_and_track(
                    board, at, m.artifact_id, ver, ContextLevel.L1,
                )
                if content:
                    summary = (
                    ...
                    parts.append(
                        f"[{at.value}/{m.artifact_id} v{ver} (L1)]\n{summary}"
                    )
    ...
    async def _build_literature_context(
        task: AgentTask, knowledge_base: Any,
    ) -> str:
            ...
            for m in metas:
                ...
                for v in range(1, latest + 1):
                    l0 = await board.read_artifact(
                        at, m.artifact_id, v, ContextLevel.L0,
                    )
                    if l0 and isinstance(l0, str):
                        chain.append(f"v{v}: {l0}")
                if chain:
                    parts.append(
                        f"[{at.value}/{m.artifact_id}]\n" + "\n".join(chain)
                    )
    ...
    async def _load_and_track(
        ...
        content = await board.read_artifact(
            artifact_type, artifact_id, version, level,
        )
